...
DETECTION_RATE=0.5          # The probability of detecting the landmark
...
PLOT_GT_LINE=True          # Line between the groundtruth and the estimation
...
PLOT_ERROR = False           # Plot the error line
STORE_STATES_FREQ = 100     # Store states every STORE_STATES_FREQ iteration for plotting latter

def endCondition(x):
    if(MOVING_FORWARD):
        return x < 1000 * N_LANDMARKS
    else:
        return x > 0.0
...
def simulate():
    ...
    if MOVING_FORWARD:
        direction_true = 1.0
        l_true_list = array([ (i * 1000) for i in range(1,N_LANDMARKS+1) ])
    else:
        direction_true = -1.0
        l_true_list = array([ 1000 * N_LANDMARKS - (i * 1000) for i in range(1,N_LANDMARKS) ])
    l_noise = (2.0 * random.rand(l_true_list.shape[0]) - ones(l_true_list.shape) ) *\
            SIGMA_L * 3.0
    l_list = l_true_list + l_noise
    l_detected_list = []
    v_true = 25.0 # m/s, no acceleration
    delta_T = DELTA_T
    iter = 0
    last_l = None
    detection_range = 15.0
    f = Filter(delta_T,SIGMA_L, SIGMA_OMEGA, S_init=S_init, P_init=P_list[0])
    ...
    while endCondition(S_true):
        ...
        if(iter % STORE_STATES_FREQ == 0 or updated):
            S_list.append(f.S)
            P_list.append(f.P)
        ...
        # Plot
        if(iter % PLOT_FREQ == 0 or updated):
            clf()
            ax = gca()
            #grid(True)
            title("Iter {}".format(iter))
            title("Positions x Variance")
            ...
            plot(S_list, array(P_list), c="red", marker='x', label="Predicted")
