...
class Controller():
    ...
    def __init__(self):
        ...
        self.velocity = 0.0
        self.lin_position = 8000.0
        self.R = 4000.0/numpy.pi
        self.spawn_x = -170.612540964
        self.spawn_z = 0.0
        ...
        self.curr_dummy_idx = len(self.position_dummies)-1 # Follows dummies on the road
    ...
    def spanTargetPositionDummies(self):
        ...
        X_sc_1 = numpy.linspace(0 + offset, 2.0 * R, N/skip, endpoint=True)
        Y_sc_1 = numpy.sqrt( (8000.0/numpy.pi) * X_sc_1 - numpy.square(X_sc_1) ) # Y values for the first semi-circle
        X_sc_2 = numpy.linspace(2.0 * R, 4.0 * R, N/skip, endpoint=False)
    ...
    def getTargetSteering(self):
        ...
        target_angle = ((4. * numpy.pi) + numpy.arctan2(delta_y, delta_x) ) % (2. * numpy.pi)
        heading_angle = ((4. * numpy.pi) + self.heading ) % (2. * numpy.pi)
